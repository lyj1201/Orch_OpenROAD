...
Rwr_ManAddTimeCuts( pManRwr, Abc_Clock() - clk );
    ...
    Abc_NtkForEachNode( pNtk, pNode, i )
    {
        ...
        //printf("nGain: %d\n", nGain);
        // write node id and nGain into a csv file
        fprintf(fpt, "%d, %d\n", pNode->Id, nGain);
...
//    Abc_AigCheckFaninOrder( pNtk->pManFunc );
    ...
    if ( !Abc_NtkCheck( pNtk ) )
    {
        ...
        printf( "Abc_NtkRewrite: success : %d; fail : %d\n", success, fail );
    return 1;
}



int Abc_NtkRewrite2( Abc_Ntk_t * pNtk, Vec_Int_t **pGain, int fUpdateLevel, int fUseZeros, int fVerbose, int fVeryVerbose, int fPlaceEnable )
{
    extern void           Dec_GraphUpdateNetwork( Abc_Obj_t * pRoot, Dec_Graph_t * pGraph, int fUpdateLevel, int nGain );
    ProgressBar * pProgress;
    Cut_Man_t * pManCut;
...
Rwr_ManAddTimeCuts( pManRwr, Abc_Clock() - clk );
    ...
    printf("nNodes: %d\n", nNodes);
    if ( pGain ) *pGain = Vec_IntAlloc(1);
    // (* pGain)->nSize = nNodes;
    // Vec_IntGrow((* pGain), nNodes);
    ...
    Abc_NtkForEachNode( pNtk, pNode, i )
    {
        ...
        // write node id and nGain into a csv file
        fprintf(fpt, "%d, %d\n", pNode->Id, nGain);
        // put nGain in the vector
        Vec_IntPush( (*pGain), nGain);
        //(* pGain)->pArray[i] = nGain;
        //printf("size of vector no Graph: %d\n", (**pGain).nSize);
