...
int Abc_NtkRewrite4( Abc_Ntk_t * pNtk, Vec_Int_t **pGain_rw, int fUpdateLevel, int fUseZeros, int fVerbose, int fVeryVerbose, int fPlaceEnable )
{
...
/******Function******************************************
 Refactor
********************************************************/
word * Abc_NodeConeTruth_2( Vec_Ptr_t * vVars, Vec_Ptr_t * vFuncs, int nWordsMax, Abc_Obj_t * pRoot, Vec_Ptr_t * vLeaves, Vec_Ptr_t * vVisited )
{   
    Abc_Obj_t * pNode;
...
int Abc_NodeConeIsConst0_2( word * pTruth, int nVars )
{
...
int Abc_NodeConeIsConst1_2( word * pTruth, int nVars )
{
    int k, nWords = Abc_Truth6WordNum( nVars );
    for ( k = 0; k < nWords; k++ )
        if ( ~pTruth[k] )
...
Dec_Graph_t * Abc_NodeRefactor_2( Abc_ManRef_t * p, Abc_Obj_t * pNode, Vec_Ptr_t * vFanins, int fUpdateLevel, int fUseZeros, int fUseDcs, int fVerbose )
{
...
clk = Abc_Clock();
    pTruth = Abc_NodeConeTruth_2( p->vVars, p->vFuncs, nWordsMax, pNode, vFanins, p->vVisited );
p->timeTru += Abc_Clock() - clk;
    if ( pTruth == NULL )
        return NULL;
    if ( Abc_NodeConeIsConst0_2(pTruth, nVars) || Abc_NodeConeIsConst1_2(pTruth, nVars) )
    {
        p->nLastGain = Abc_NodeMffcSize( pNode );
        p->nNodesGained += p->nLastGain;
        p->nNodesRefactored++;
        return Abc_NodeConeIsConst0_2(pTruth, nVars) ? Dec_GraphCreateConst0() : Dec_GraphCreateConst1();
    }
clk = Abc_Clock();
...
Abc_ManRef_t * Abc_NtkManRefStart_2( int nNodeSizeMax, int nConeSizeMax, int fUseDcs, int fVerbose )
{   
    Abc_ManRef_t * p;
...
void Abc_NtkManRefStop_2( Abc_ManRef_t * p )
{   
...
void Abc_NtkManRefPrintStats_2( Abc_ManRef_t * p )
{   
...
int Abc_NtkRefactor3_1( Abc_Ntk_t * pNtk, Vec_Int_t **pGain_ref, int nNodeSizeMax, int nConeSizeMax, int fUpdateLevel, int fUseZeros, int fUseDcs, int fVerbose )
{
    extern int           Dec_GraphUpdateNetwork( Abc_Obj_t * pRoot, Dec_Graph_t * pGraph, int fUpdateLevel, int nGain );
    ProgressBar * pProgress;
    ...
    pManRef = Abc_NtkManRefStart_2( nNodeSizeMax, nConeSizeMax, fUseDcs, fVerbose );
...
clk = Abc_Clock();
        pFForm = Abc_NodeRefactor_2( pManRef, pNode, vFanins, fUpdateLevel, fUseZeros, fUseDcs, fVerbose );
...
pManRef->timeTotal = Abc_Clock() - clkStart;
    ...
    if ( fVerbose )
        Abc_NtkManRefPrintStats_2( pManRef );
    Abc_NtkManCutStop( pManCut );
    Abc_NtkManRefStop_2( pManRef );
    Abc_NtkReassignIds( pNtk );
    if ( RetValue != -1 )
    {
        if ( fUpdateLevel )
...
int Abc_NtkResubstitute3_1( Abc_Ntk_t * pNtk, Vec_Int_t **pGain_res, int nCutMax, int nStepsMax, int nLevelsOdc, int fUpdateLevel, int fVerbose, int fVeryVerbose )
{
    extern int           Dec_GraphUpdateNetwork( Abc_Obj_t * pRoot, Dec_Graph_t * pGraph, int fUpdateLevel, int nGain );
    ProgressBar * pProgress;
...
void Abc_ManResubCollectDivs_rec1_1( Abc_Obj_t * pNode, Vec_Ptr_t * vInternal )
{
    // skip visited nodes
    ...
    // collect the fanins
    Abc_ManResubCollectDivs_rec1_1( Abc_ObjFanin0(pNode), vInternal );
    Abc_ManResubCollectDivs_rec1_1( Abc_ObjFanin1(pNode), vInternal );
...
int Abc_ManResubCollectDivs( Abc_ManRes_t * p, Abc_Obj_t * pRoot, Vec_Ptr_t * vLeaves, int Required )
{
    ...
    Abc_ManResubCollectDivs_rec1_1( pRoot, p->vDivs );
...
Dec_Graph_t * Abc_ManResubQuit0_2( Abc_Obj_t * pRoot, Abc_Obj_t * pObj )
{
    Dec_Graph_t * pGraph;
    Dec_Edge_t eRoot;
...
Dec_Graph_t * Abc_ManResubQuit1_2( Abc_Obj_t * pRoot, Abc_Obj_t * pObj0, Abc_Obj_t * pObj1, int fOrGate )
{
    Dec_Graph_t * pGraph;
...
Dec_Graph_t * Abc_ManResubQuit21_2( Abc_Obj_t * pRoot, Abc_Obj_t * pObj0, Abc_Obj_t * pObj1, Abc_Obj_t * pObj2, int fOrGate )
{
    Dec_Graph_t * pGraph;
...
Dec_Graph_t * Abc_ManResubQuit2_2( Abc_Obj_t * pRoot, Abc_Obj_t * pObj0, Abc_Obj_t * pObj1, Abc_Obj_t * pObj2, int fOrGate )
{
    Dec_Graph_t * pGraph;
...
Dec_Graph_t * Abc_ManResubQuit3_2( Abc_Obj_t * pRoot, Abc_Obj_t * pObj0, Abc_Obj_t * pObj1, Abc_Obj_t * pObj2, Abc_Obj_t * pObj3, int fOrGate )
{
    Dec_Graph_t * pGraph;
...
//            if ( puData[w] != puDataR[w] )
            if ( (puData[w] ^ puDataR[w]) & p->pCareSet[w] ) // care set
                break;
        if ( w == p->nWords )
            return Abc_ManResubQuit0_2( p->pRoot, pObj );
    }
    return NULL;
}
...
Dec_Graph_t * Abc_ManResubDivs1( Abc_ManRes_t * p, int Required )
{
    ...
    //                if ( (puData0[w] | puData1[w]) != puDataR[w] )
            ...
            {
                p->nUsedNode1Or++;
                return Abc_ManResubQuit1_2( p->pRoot, pObj0, pObj1, 1 );
            }
        }
    }
    ...
    //                if ( (puData0[w] & puData1[w]) != puDataR[w] )
            ...
            {
                p->nUsedNode1And++;
                return Abc_ManResubQuit1_2( p->pRoot, pObj0, pObj1, 0 );
            }
        }
    }
    return NULL;
...
Dec_Graph_t * Abc_ManResubDivs12( Abc_ManRes_t * p, int Required )
{
    ...
    //                    if ( (puData0[w] | puData1[w] | puData2[w]) != puDataR[w] )
                ...
                if ( w == p->nWords )
                {
                    ...
                    assert(pObjMin0);
                    assert(pObjMin1);
                    return Abc_ManResubQuit21_2( p->pRoot, pObjMin0, pObjMin1, pObjMax, 1 );
                }
            }
        }
    }
    ...
    //                    if ( (puData0[w] & puData1[w] & puData2[w]) != puDataR[w] )
                ...
                if ( w == p->nWords )
                {
                    ...
                    assert(pObjMin0);
                    assert(pObjMin1);
                    return Abc_ManResubQuit21_2( p->pRoot, pObjMin0, pObjMin1, pObjMax, 0 );
                }
            }
        }
    }
...
Dec_Graph_t * Abc_ManResubDivs2( Abc_ManRes_t * p, int Required )
{
    ...
    //                    if ( (puData0[w] | (puData1[w] & puData2[w])) != puDataR[w] )
            ...
            {
                p->nUsedNode2OrAnd++;
                return Abc_ManResubQuit2_2( p->pRoot, pObj0, pObj1, pObj2, 1 );
            }
        }
    }
    // check negative unate divisors
    ...
    //                    if ( (puData0[w] & (puData1[w] & puData2[w])) != puDataR[w] )
            ...
            {
                p->nUsedNode2AndOr++;
                return Abc_ManResubQuit2_2( p->pRoot, pObj0, pObj1, pObj2, 0 );
            }
        }
    }
    return NULL;
...
//                    if ( ((puData0[w] | puData1[w]) | (puData2[w] | puData3[w])) != puDataR[w] )
            ...
            {
                p->nUsedNode3OrAnd++;
                return Abc_ManResubQuit3_2( p->pRoot, pObj0, pObj1, pObj2, pObj3, 1 );
            }
        }
    }
...
int Abc_CutVolumeCheck_rec_2( Abc_Obj_t * pObj )
{
    ...
    return 1 + Abc_CutVolumeCheck_rec_2( Abc_ObjFanin0(pObj) ) +
        Abc_CutVolumeCheck_rec_2( Abc_ObjFanin1(pObj) );
}
/**Function*************************************************************
...
int Abc_CutVolumeCheck( Abc_Obj_t * pNode, Vec_Ptr_t * vLeaves )
{
    ...
    return Abc_CutVolumeCheck_rec_2( pNode );
}
...
void Abc_CutFactor_rec_2( Abc_Obj_t * pObj, Vec_Ptr_t * vLeaves )
{
    if ( pObj->fMarkA )
        return;
    ...
    Abc_CutFactor_rec_2( Abc_ObjFanin0(pObj), vLeaves );
    Abc_CutFactor_rec_2( Abc_ObjFanin1(pObj), vLeaves );
}
...
Vec_Ptr_t * Abc_CutFactor_2( Abc_Obj_t * pNode )
{
    Vec_Ptr_t * vLeaves;
    Abc_Obj_t * pObj;
    int i;
    ...
    Abc_CutFactor_rec_2( Abc_ObjFanin0(pNode), vLeaves );
    Abc_CutFactor_rec_2( Abc_ObjFanin1(pNode), vLeaves );
    Vec_PtrForEachEntry( Abc_Obj_t *, vLeaves, pObj, i )
        pObj->fMarkA = 0;
    return vLeaves;
}
...
Vec_Ptr_t * Abc_CutFactorLarge( Abc_Obj_t * pNode, int nLeavesMax )
{
    ...
    Vec_PtrForEachEntry( Abc_Obj_t *, vLeaves, pLeaf, i )
    {
        ...
        if ( Abc_ObjIsCi(pLeaf) )
            ...
            Vec_PtrPush( vFactors, Abc_CutFactor_2(pLeaf) );
    }
...
// orchestration with a pre-defined operatin list -- DesisionList
int Abc_NtkOrchEvaRL( Abc_Ntk_t * pNtk, Vec_Int_t **pGain_rwr, Vec_Int_t **pGain_res,Vec_Int_t **pGain_ref, Vec_Int_t **DecisionList, Vec_Int_t **DecisionMask, char * DecisionFile, int fUseZeros_rwr, int fUseZeros_ref, int fPlaceEnable, int nCutMax, int nStepsMax, int nLevelsOdc, int fUpdateLevel, int fVerbose, int fVeryVerbose, int nNodeSizeMax, int nConeSizeMax, int fUseDcs )
{
    extern int           Dec_GraphUpdateNetwork( Abc_Obj_t * pRoot, Dec_Graph_t * pGraph, int fUpdateLevel, int nGain );
    ProgressBar * pProgress;
    // For resub
...
// Update the graph with pre-defined order!
      //printf("id before orch1: %d\n", pNode->Id);
      if ( (**DecisionList).pArray[iterNode] == 0)
      {
// rewrite
        fUseZeros_rwr = 0;
...
Rwr_ManAddTimeUpdate( pManRwr, Abc_Clock() - clk );
     ...
     // printf("id before orch2: %d\n", pNode->Id);
       if ( (**DecisionList).pArray[iterNode] == 1)
       {
        fUseZeros_rwr = 1;
        nGain = Rwr_NodeRewrite( pManRwr, pManCutRwr, pNode, fUpdateLevel, fUseZeros_rwr, fPlaceEnable );
        Vec_IntPush( (*pGain_rwr), nGain);
...
Rwr_ManAddTimeUpdate( pManRwr, Abc_Clock() - clk );
       ...
       if ((**DecisionList).pArray[iterNode] == 2)
       {
...
pManRes->timeNtk += Abc_Clock() - clk;
       ...
       if ((**DecisionList).pArray[iterNode] == 3)
       {
        fUseZeros_ref = 0;
        vFanins = Abc_NodeFindCut( pManCutRef, pNode, fUseDcs );
pManRef->timeCut += Abc_Clock() - clk;
clk = Abc_Clock();
        pFFormRef = Abc_NodeRefactor_2( pManRef, pNode, vFanins, fUpdateLevel, fUseZeros_ref, fUseDcs, fVerbose );
pManRef->timeRes += Abc_Clock() - clk;
...
pManRef->timeNtk += Abc_Clock() - clk;
       ...
       if ((**DecisionList).pArray[iterNode] == 4)
       {
        fUseZeros_ref = 1;
        vFanins = Abc_NodeFindCut( pManCutRef, pNode, fUseDcs );
pManRef->timeCut += Abc_Clock() - clk;
...
pManRef->timeNtk += Abc_Clock() - clk;
        Dec_GraphFree( pFFormRef );
        (*DecisionMask)->pArray[iterNode] = 4;
        ops_refz++;
        continue;
        }
       }
